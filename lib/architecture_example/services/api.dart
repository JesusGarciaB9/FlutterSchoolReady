...
Future<List<Person>> getPersonas(String username, String token, int courseId) async {
    Uri uri = Uri.https(
      "movil-api.herokuapp.com",
      '$username/courses/$courseId', 
    );
    ...
    print(
        ...
        print(response.body);
    if (response.statusCode == 200) {
      return CourseDetail.fromJson(json.decode(response.body)).students;
    } else {
      //Map<String, dynamic> body = json.decode(response.body);
      //String error = body['error'];
      //print('error  $error');
...
Future<Person> getStudent(String username, String token, int studentId) async {
    Uri uri = Uri.https(
      "movil-api.herokuapp.com",
    ...
    print(
        'getstudent username $username token $token => ${response.statusCode}');
        print(response.body);
    if (response.statusCode == 200) {
  ...
  Future<Person> getProfesor(String username, String token, int profesorId) async {
    Uri uri = Uri.https(
      "movil-api.herokuapp.com",
      '$username/professors/$profesorId', 
    );
    final http.Response response = await http.get(
      uri,
      headers: <String, String>{
  ...
  Future<Person> addPerson(String username, String token, int courseId) async {
  final http.Response response = await http.post(
  ...
  if (response.statusCode == 200) {
    ...
    Map<String, dynamic> body = json.decode(response.body);
    String error = body['error'];
    print('error  AHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHH');
    return Future.error(error);
  }
}  
